...
public class VeinCountPlacement extends PlacementModifier {
    public static final int MAX_VEINS = 2;
    ...
    public Stream<BlockPos> getPositions(PlacementContext context, RandomSource random, BlockPos pos) {
        ...
        Set<Integer> selected = selectedVeins.computeIfAbsent(chunkPos, chunk -> {
            ...
            for (int i = 0; i < Math.min(MAX_VEINS, totalVeins); i++) {
                int j = i + chunkRandom.nextInt(totalVeins - 1);
                Collections.swap(indices, i, j);
            }
            return new HashSet<>(indices.subList(0, Math.min(MAX_VEINS, totalVeins)));
        });
